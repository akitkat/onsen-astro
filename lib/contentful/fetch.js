...
const getPostContent = entry => {
  const body = entry.fields.body.replace(/hotel-info-item/g, 'HotelInfoItem')
    ...
    .replace(/<golf-course-item\b[^>]*\/>/g, '')
    .replace(/<golf-course-item\b[^>]*>[\s\S]*?<\/golf-course-item>/g, '')
...
const golfTagIds = ['3pDpxA3gOgQjrKLA03AyQl', '2vrqM3i0zgxsI0USw5nbFN', '28c5AuaQ1WYjGh5WOUOc1A'];
const kodureHotelTagIds = ['6pMsFHkQF6EzzIjmtUjpiT'];
const saunaTagIds = ['2waXv8wHgHCRFX1wL9Rgfx'];
const sakeTagIds = ['4AAjDFEY1E9CyZzA2szkLl'];
const ignoreTagIds = [...golfTagIds, ...kodureHotelTagIds, ...saunaTagIds, ...sakeTagIds].join(',');

(async () => {
  const total = await client.getEntries({
    content_type: 'post',
    'fields.tags.sys.id[nin]': ignoreTagIds,
    limit: 1
  }).then(res => res.total)
